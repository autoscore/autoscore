...
Unfortunately, `autoscore` double counts words that appear twice in a single target sentence. Many sentences in the field do not have this issue but some do. For those that do, you can use the new `double_word_detect()` function to find the targets that have the double words. For example:
